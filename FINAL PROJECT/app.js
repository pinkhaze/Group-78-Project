...
app.get('/provided-utilities', async (req, res) => {
        const query1 = `
            SELECT 
                ProvidedUtilities.utility_ID, 
                CONCAT(UtilityProviders.name) AS utility_provider_name,
                CONCAT(Units.unit_number) AS unit_number,
                Units.is_available, 
                Units.num_bedroom, 
                Units.num_bathroom, 
                Units.square_feet, 
                Units.rent_price, 
                UtilityProviders.name AS provider_name,
...
app.post('/add-maintenance-request-form', function(req, res) {
    ...
    let isClosed = (parseInt(data.is_closed));
...
app.put('/update-maintenance-request', function(req, res) {
    ...
    let isClosed = (parseInt(data.isClosed));
